"""Client-side helpers for instrumenting an agentic task.

An app points its LLM client's ``base_url`` at the gateway and wraps a run in
``task(...)``. Every request made through a ctrlrtn-wrapped httpx client inside
...
while a task is active raises instead of being silently recorded as
baseline.
"""
...
from ctrlrtn.sdk.lifecycle import Step, Task, ToolOperation, task
