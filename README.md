...
as well, and did switching pay off.
...
## Sixty seconds to your first numbers
...
```bash
export ANTHROPIC_BASE_URL=http://127.0.0.1:4000   # or OPENAI_BASE_URL
```
...
(`fp:...`). The Python SDK sets all of this for you; see
[instrument a workflow](docs/instrument-a-workflow.md). Then look:
...
[Getting started](docs/getting-started.md) walks through this with a real
application.
...
`routing.yaml` and activated atomically, so the control plane has a Git
history. [Run an experiment](docs/run-an-experiment.md) covers each step,
and [evaluation](docs/evaluation.md) explains why the verdicts can be trusted
and what they do not claim.

## A real run

A multi-agent newspaper built on [Hugin](https://github.com/arnovich/gimle-hugin),
a few dozen editions of real traffic recorded, each Sonnet role replayed on
Haiku and judged blind against the source data:
...
The editor switches: a mean difference of +0.07 points with a lower bound of
−0.09, indistinguishable from Sonnet at a fraction of the price. The
journalist stays on Sonnet: a mean difference of −1.18 with a lower bound of
−1.75, well past the one-point margin. The tempting saving on the most
expensive role is exactly the switch the router exists to refuse, so the
aggregate here is $1.04 of $23.23. The verdict is per role, and it differs.

Method: 40 pairings per role over more than 20 independent editions,
blinded position-swapped judging, paired non-inferiority test with a
1.0-point margin at 95 % one-sided confidence. Repricing assumes the same
token mix, an upper bound. The table and chart are generated; reproduce them
on your own workload with [the campaign guide](docs/campaign.md).
...
- [Console](docs/console.md), [configure](docs/configure.md),
  [deploy](docs/deploy.md), [architecture](docs/architecture.md).
