...
- `ANTHROPIC_API_KEY` is set in the shell that runs replays and judges. The
  replay path and the judge (`claude-opus-4-8` by default, `--judge-model`)
  call the Anthropic API directly, not through the proxy, whatever provider
  the application uses. Recorded prompts and outputs leave the box on those
  calls.
...
`NON-INFERIOR`, `NOT non-inferior`, or `UNDERPOWERED (cannot conclude)`; in
the JSON artifact the same verdicts are `NON_INFERIOR`, `NOT_NON_INFERIOR`
and `UNDERPOWERED`. An underpowered result means the lower confidence bound did not clear the
...
Each mirrored call and its actual call share a pair id, so the console can
show them side by side. Mirrors dropped by a full queue or failed at the
provider are counted as attrition instead of disappearing. Shadowing sends live data to the candidate provider
and bills its credentials.

## 4. Split live traffic

A live experiment serves the candidate to a share of the use-case's tasks.
Each task is assigned to one arm, the baseline model or the candidate, and
the assignment is sticky, so every call of one job sees the same model:
...
the failure rate the application reported per arm, and cannot certify a
fine quality margin. Its verdicts and exit codes are tabulated in
[Evaluation](evaluation.md). It needs at least 30 tasks with a reported
...
The note is stored with the route and shown by `route list` and the console.
...
one. [Configuration](configure.md) describes the `providers` block.
...
file in the repository where you keep operational configuration and name
that repository with `--repo`:
...
Rolling back is a commit that removes the route, then `activate` again.
...
shows the drift. A route to a provider with a different API fails every
call of that use-case until it is cleared. Never commit credentials or
recordings to that repository; `note` values are stored and displayed.
