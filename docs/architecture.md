...
| `sdk/` | Client instrumentation: task, route, and workflow context, httpx request stamping, task, step, and tool-operation context managers, event transport, and a late-bound reporting seam | `context.py`, `http.py`, `lifecycle.py`, `reporting.py`, `dispatch.py` |
