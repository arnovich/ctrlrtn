...
This is the header and event contract that the SDK and the gateway implement.
It does not authorize the router to schedule, merge, fuse, skip, or parallelize
work.
...
they do not describe which call depends on which. This contract makes workflow identity explicit
so recorded calls can be grouped, attributed, visualized, and routed at step
scope without inferring control facts from prompt text.
...
is `(task_id, step_run_id)`. Neither `task_id` nor `step_run_id` is a routing
target.
...
All referenced runs must share its `task_id`; cross-task relationships are not
expressed in these fields.
...
- identifiers use the ASCII grammar `[A-Za-z0-9][A-Za-z0-9._:/-]*`; empty
  values, control characters, duplicate dependency IDs, a run that depends on
  itself, an attempt below one, and identifiers over their limit are rejected
  from workflow attribution;
- malformed metadata never breaks ordinary proxying: the gateway records it as
  a propagation diagnostic and forwards the request. Strict SDK mode raises
  before sending;
- the gateway never derives a routable step identity from request bodies.

Limits are defined once in `ctrlrtn.workflow.identity` and shared by the SDK
and the gateway: 128 ASCII bytes for names, versions, and run IDs; at most 32
dependencies; at most 4 KiB for the complete workflow header set.

ctrlrtn headers are router control metadata. The proxy records their normalized
values in dedicated columns and removes all `x-ctrlrtn-*` headers before
contacting an upstream provider. Internal graph names and identifiers are never
disclosed to model providers merely because the client supplied them.

## SDK contract

The API extends the context-variable model of `x-ctrlrtn-task` and
`x-ctrlrtn-route`:
...
Context variables propagate naturally to async child tasks. `sdk.bind()` copies
the complete workflow and step context across thread boundaries. A subprocess
or another service receives an explicit serializable carrier from
`sdk.export_carrier()` and imports it with `sdk.import_carrier()` after
validation; Python context copying is not a cross-process protocol. Creating
concurrent children from one captured context requires opening a distinct
`step()` inside each child so run IDs are not accidentally shared.
...
whether a step with no model call completed. The gateway therefore accepts
lifecycle events on the host-protected local endpoint
`POST /ctrlrtn/workflow-events`, with the same deployment trust boundary as
`/ctrlrtn/outcome`. An event contains:
...
`skipped` without provider calls. Outcome fields (`success`, `score`,
`error_code`) are valid only on a terminal event. Repeated identical events are
harmless. Conflicting terminal events mark the run inconsistent rather than
choosing the more favorable outcome.
...
SHA-256 digest. Activation records both the source revision and document
...
inputs. Accuracy is measured against explicitly instrumented workflows.

The implemented inference method is `tool-id-link/v1`. It runs only when the
operator invokes `ctrlrtn workflow infer`. Within one task it matches an
Anthropic `tool_use.id` or OpenAI `tool_calls[].id` in a recorded response to
the first later request carrying the corresponding `tool_use_id`/`tool_call_id`.
...
assignment. `ctrlrtn workflow inferred` lists the persisted edges and
`ctrlrtn workflow diagnostics` shows malformed or conflicting explicit facts.

### Discovering undeclared workflow families
...
recurring role/tool-shape multiset, not a dependency chain. Single-link clustering
...
`workflow identify FAMILY WORKFLOW VERSION --output NEW.json` turns one
...
`workflow discover --background` freezes the inputs and queues a durable job
for `ctrlrtn worker`. The worker performs bounded pairwise clustering with
durable progress, cooperative cancellation, restart recovery, and frozen-input
digest verification. Its checksummed result contains no trace IDs or bodies and
remains analysis-only. Each eligible task digest has an assigned or unclustered
record with its family, representative similarity, structural variant
fingerprint, trace count, explicit/synthetic correlation source, ambiguity, and
reason. `workflow discovery-compare OLD NEW` matches families one-to-one by
normalized node/edge shape before reporting support and membership changes,
because representative-derived family IDs may legitimately change as new
variants arrive. Differing algorithms or parameters remain visibly incompatible
rather than being silently normalized.

The result also binds model, provider, token, cost, latency, and HTTP status
fields into the frozen evidence digest and derives a passive projection for
each family: representative timelines, path frequencies, tools, retries/loops,
branches/joins, and aggregate operational metrics, plus digest-only per-task
timelines and metrics. Because HTTP success is not a logical workflow outcome,
every passive member remains in the visible unknown-outcome denominator.
`workflow discovery-project JOB FAMILY` exports dotted-edge Mermaid or
authority-labelled JSON without raw task IDs or bodies.

A background job carries an immutable discovery scope over task last-observed
time, provider, served model, experiment, and arm (`--since`, `--until`,
`--provider`, `--model`, `--experiment`, `--arm`). A cohort match includes
every call of the task rather than deleting non-matching calls from its graph.
...
rejected as unrelated unless `--allow-unrelated` is given. This makes
cross-arm and cross-model analysis explicit without claiming that a
structurally matched inferred family is a declared workflow version.
The result records the bounded-selection denominator and strategy, including
...
makes pruned source evidence visible and blocks ordinary `jobs export` unless
`--allow-invalidated` is given.

The console exposes the same operations (`u`, `f`, `i`, `y`, `j`; see
[console.md](console.md)). Its long-lived trace store remains read-only;
explicit queue and proposal actions use narrow writes, and proposal files must
not already exist.

A declared definition is descriptive rather than executable. Its canonical form
is the `workflows` section of the Git-backed routing configuration
([configure.md](configure.md)): a workflow name, exact version, and stable step
...
than an expression for the router to evaluate. Canonicalization and hashing are
deterministic, and unknown keys fail validation rather than being ignored.

## Graph and diagram representation

Normalized persisted nodes and typed edges are the canonical representation.
The console renders a time-ordered step-run list with status, indentation and
edge markers, and a selected-node detail pane; this remains usable in narrow
terminals and for loops too dense for a chart. Mermaid `flowchart` is the
review and export format because it is textual and Git-diffable. IDs are
generated from internal keys and all user labels are escaped, so labels cannot
inject Mermaid syntax. Mermaid is a projection, never imported as authoritative
state.

The console's workflows table opens a per-task timeline with step status,
attempts, calls, cost, provider latency, and models. Solid `->` links are
explicit application facts; dotted `~>` links are persisted analysis-only
inference. `ctrlrtn workflow diagram TASK` exports the same graph as
deterministic Mermaid, using opaque node IDs and escaped labels; `--output
PATH` writes it for review or Git history. Neither view is an orchestration
definition or an import format.

The aggregate flow/Sankey projection covers one exact workflow version at a
time. It reports stable-step task/run/call/failure counts, cost and latency,
plus edge task counts and source-task branching denominators. Explicit and
inferred edges are separate in text; the Mermaid Sankey intentionally contains
explicit application facts only. The selected console workflow shows this
aggregate beneath its per-task timeline, and `ctrlrtn workflow flow WORKFLOW
VERSION --output FLOW.mmd` exports it. The projection is analysis-only.

## Outcome attribution

Outcomes are explicit measurements, not conclusions about cause. The task outcome
posted to `/ctrlrtn/outcome` remains workflow-run scoped. The event API adds
...
A workflow outcome is never copied onto every step. A step inherits no score
...
the last step does not show that the step caused the failure.
...
states are marked inconsistent and excluded from success/score statistics. A
run whose ID maps to more than one stable identity is excluded entirely and
surfaced by diagnostics. `--workflow NAME --version REVISION` narrows the
report; it never mixes versions.

Tool operations have a provider-independent application identity: the exact
workflow step plus a stable logical `operation_id`, a unique per-attempt ID and
positive attempt number, an operation name, and one conservative effect
contract (`unknown`, `pure`, `idempotent`, or `stateful`). Missing knowledge is
...
`cancelled`) tool-operation events through the SDK, which posts them to
`POST /ctrlrtn/tool-operation-events`. Terminal events require explicit
success and may report a bounded error code, latency, and cost. The gateway
persists these separately, and connected step detail labels them as explicit
application facts. Provider-owned tool IDs remain inference evidence and cannot
authorize batching, fusion, retry collapse, or execution changes.

`ctrlrtn workflow recommendations` derives review prompts from explicit step
metrics and committed descriptive definitions. It can flag expensive steps,
repeated model calls, declared siblings worth investigating for parallel
execution, and single-successor chains worth reviewing for possible fusion.
...
fuse, parallelize, or otherwise change a workflow.
...
revision. Unknown versions and incomplete workflow metadata fall back to
use-case behavior; they do not approximately match another version.

Offline replay, online shadow, and split experiments may select a stable step
with `--workflow`, `--workflow-version`, and `--step` together. Their evidence
remains step scoped unless the candidate owns the whole workflow and a workflow
outcome measures it. A candidate continuing a baseline-generated history is not
evidence of end-to-end workflow equivalence. Scoped tripwires use explicit step
lifecycle outcomes and never inherit the task outcome.

Clients with only `x-ctrlrtn-task` and `x-ctrlrtn-route` remain task and
use-case observable but have no authoritative workflow graph. All workflow
columns are additive and nullable. Historical traces can receive inferred
annotations, but never fabricated explicit identity.

## Security, privacy, and retention

Workflow headers are assertions from the calling application, not authentication
or authorization. The same-host deployment and Host checks are the trust
boundary. A remotely exposed gateway needs authenticated client identity before
workflow metadata can authorize routing or outcome submission.

Identifiers must not contain secrets, user text, prompt excerpts, credentials,
or personal data. Raw errors stay in protected trace storage; lifecycle events
use bounded machine-readable error codes. Configuration repositories hold
...
runtime graph, inferred facts, and generated diagrams. Aggregates are either
recomputed or documented as non-reversibly anonymized.
...
validation, visualization, attribution, routing at declared scope, and
optimization recommendations. It does not permit the router to reorder, run,
merge, fuse, skip, cache, retry, or parallelize application steps. Tool calls
are stateful and non-idempotent unless explicitly declared otherwise.
